...
def create_metadata_h5(h5_file, meta_name, list_meta_field, matching_field, meta_individuals, num_tiles, key_dict,
                       override, frame):
    h5_metadata_path = h5_file.replace('.h5', '_%s.h5' % meta_name)
    if override and os.path.isfile(h5_metadata_path):
    ...
    with h5py.File(h5_file, 'r') as orig_content:
        ...
        for i in range(set_dict[list(storage_dict.keys())[0]].shape[0]):
            ...
            index += 1
        print()
    return h5_metadata_path

def include_metadata(meta_file, meta_name, list_meta_field, matching_field, h5_file, override=False):
    ...
    meta_h5_file=create_metadata_h5(h5_file, meta_name, list_meta_field, matching_field, meta_individuals, num_tiles, key_dict,
                       override, frame)
    return meta_h5_file
